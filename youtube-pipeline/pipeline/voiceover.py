...
API key). Also captures word-boundary timings so video_builder.py can build
accurately-timed on-screen captions instead of guessing from a words-per-
minute average.
"""
import asyncio
...
def generate_voiceover(cfg: dict, narration: str, audio_path: str, elevenlabs_api_key: str | None) -> Voiceover:
    ...
    if provider == "elevenlabs":
        ...
        # ElevenLabs doesn't return word timings via this endpoint; captions
        # fall back to an even word-per-minute estimate in video_builder.py.
