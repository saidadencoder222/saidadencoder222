...
past `followup_after_days` with no reply get a single follow-up, capped
at `max_touches` total per lead. Meant to be invoked periodically (e.g.
once a day via cron/systemd timer) rather than looped in-process.
"""
...
def _due_for_next_touch(conn, lead) -> int:
    sent = touches_sent(conn, lead["place_id"])
    if sent >= CONFIG.max_touches:
        return None
    if sent == 0:
        return 1

    last = last_sent_at(conn, lead["place_id"])
...
def run_campaign_batch(sender_email: str):
    ...
    with connect(CONFIG.db_path) as conn:
        for lead in leads_with_email(conn):
            ...
            subject = _render(subject_tpl, lead)
            ...
            sent_this_run += 1
            print(f"Sent touch {touch} to {lead['name']} <{lead['email']}>")
